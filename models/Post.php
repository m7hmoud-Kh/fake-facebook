...
class Post
{
    ...
    {
        $stmt = $this->con->prepare('SELECT posts.id as post_id, posts.*, users.id as `user_id` ,users.* FROM posts JOIN users on posts.user_id = users.id WHERE posts.user_id IN (SELECT friends.friend_id FROM friends WHERE friends.user_id = ?) OR users.id = ? ORDER BY posts.id DESC; ');
